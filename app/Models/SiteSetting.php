...
class SiteSetting extends Model
{
    ...
    /**
     * Get all settings grouped by group (with locale support and all translations)
     */
    public static function getGrouped(?string $locale = null)
    {
        ...
        // Apply locale to each setting and add all translations
        foreach ($settings as $setting) {
            if (in_array($setting->key, $translatableKeys)) {
                // Get all translations for this setting
                $translations = [
                    'en' => $setting->value, // English is stored in main value field
                    'es' => $setting->getTranslation('value', 'es') ?: '',
                    'pt' => $setting->getTranslation('value', 'pt') ?: '',
                ];
                
                // Add translations to setting object
                $setting->translations = $translations;
                
                // Set current locale value for display
                if ($locale !== 'en' && $translations[$locale]) {
                    $setting->value = $translations[$locale];
                }
            }
        }
