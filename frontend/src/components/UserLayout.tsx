...
const UserLayout = () => {
  ...
  // Handler for Directos - navigate to live page
  const handleDirectosClick = (e: React.MouseEvent) => {
    e.preventDefault();
      ...
      <nav className="fixed top-0 z-50 w-full bg-gradient-to-b from-black/95 to-transparent transition-all duration-300 px-4 md:px-10 py-4 flex items-center justify-between border-b border-[#39282e]/30 backdrop-blur-md">
        <div className="flex items-center gap-4 sm:gap-8">
          ...
          <div className="hidden xl:flex items-center gap-6">
              ...
              <Link
                to={getPathWithLocale("/live")}
                className={`text-xs font-bold uppercase tracking-wider transition-colors ${
                  isActive('/live') ? 'text-white' : 'text-gray-300 hover:text-white'
                }`}
              >
                ...
                <Input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && searchQuery.trim()) {
                      navigate(getPathWithLocale(`/my-list-search?search=${encodeURIComponent(searchQuery.trim())}`));
                    }
                  }}
